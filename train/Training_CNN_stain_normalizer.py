...
if (TASK=='StainNet'):
	...
	model_Net.load_state_dict(torch.load("./StainNet-master/checkpoints/StainNet/aligned_histopathology_dataset/latest_net_G_B.pth"))

elif (TASK=='StainGAN'):
	model_Net = ResnetGenerator(3, 3, ngf=64, norm_layer=torch.nn.InstanceNorm2d, n_blocks=9).to(device)
	try:
		model_Net.load_state_dict(torch.load("/./StainGAN-master/checkpoints/prostate/experiment_name/latest_net_G_A.pth"))
	except:
		state_dict_fname = "./StainGAN-master/checkpoints/prostate/experiment_name/latest_net_G_A.pth"
