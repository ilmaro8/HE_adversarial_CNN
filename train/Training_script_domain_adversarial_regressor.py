...
parser.add_argument('-t', '--TASK', help='task (binary/multilabel)',type=str, default='domain_regressor')
...
while (epoch<num_epochs and early_stop_cont<EARLY_STOP_NUM):
	...
	for inputs,labels, h_e_matrices in training_generator:
		...
		inputs, labels, h_e_matrices = inputs.to(device), labels.to(device), h_e_matrices.type(torch.FloatTensor).to(device)
